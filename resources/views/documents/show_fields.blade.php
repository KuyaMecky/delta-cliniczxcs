<div>
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade show active" id="documentOverview" role="tabpanel">
                        ...
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-3 d-flex flex-column">
                                ...
                                <p class="m-0">@if(!empty($documents->document_url))
                                        <a href="{{$documents->document_url}}" target="_blank"><span class="badge fs-6 bg-light-info">{{__('messages.document.view')}}</span></a>
                                    @else
                            ...
                            <div class="col-lg-3 d-flex flex-column">
                                ...
                                <span class="fw-bolder fs-6 text-gray-800" data-placement="top"  data-bs-original-title="{{ date('jS M, Y', strtotime($documents->created_at)) }}">{{ $documents->created_at->diffForHumans() }}</span>
                            </div>
                            <div class="col-lg-3 d-flex flex-column">
                                <label class="fw-bold text-muted py-3">{{ __('messages.common.last_updated').(':')  }}</label>
                                <span class="fw-bolder fs-6 text-gray-800" data-placement="top"  data-bs-original-title="{{ date('jS M, Y', strtotime($documents->updated_at)) }}">{{ $documents->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
